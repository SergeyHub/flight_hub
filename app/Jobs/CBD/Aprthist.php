...
class Aprthist implements ShouldQueue
{
    use Batchable,
        Dispatchable,
        InteractsWithQueue,
        ...
        if($response->successful()) {
            ...
            $crawler->filterXPath("//rowset/row")->each(function($node) use ($table, $cbdService) {
                try {
                    ...
                    {
                        \App\Models\Aprthist::updateOrCreate(['AIRPORTS_ID' => $node->filterXPath("//aprthist_id")->text()], [
                            'APRTHIST_ID' => $cbdService->nodeNotNull($node, "//aprthist_id"),
                            'AIRPORTS_ID' => $cbdService->nodeNotNull($node, "//airports_id"),
                            'ICAOLAT4' => $cbdService->nodeNotNull($node, "//icaolat4"),
                            'ISNATIONCODE' => $cbdService->nodeNotNull($node, "//isnationcode"),
                        ]);
                    });
