...
class Acft_grp implements ShouldQueue
{
    use Batchable,
        Dispatchable,
        InteractsWithQueue,
        ...
        if($response->successful()) {
            ...
            $crawler->filterXPath("//rowset/row")->each(function($node) use ($table, $cbdService) {
                try {
                    ...
                    {
                        \App\Models\Acft_grp::updateOrCreate(['ACFT_GRP_ID' => $node->filterXPath("//acft_grp_id")->text()], [
                            'ACFT_GRP_ID' => $cbdService->nodeNotNull($node, "//acft_grp_id"),
                            'ACFTGRP_ID' => $cbdService->nodeNotNull($node, "//acftgrp_id"),
